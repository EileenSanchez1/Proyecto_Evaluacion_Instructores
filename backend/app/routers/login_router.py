...
from app.config.security import crear_access_token, crear_token_recuperacion, verificar_token_recuperacion
from app.schemas.login import LoginRequest, LoginResponse, SolicitarRecuperacionRequest, RestablecerPasswordRequest
...
def login(datos: LoginRequest, session: Session = Depends(get_session)):
    ...
    token = crear_access_token(id_usuario=usuario.id_usuario, correo=usuario.correo, rol=usuario.rol.nombre)
    ...
    if usuario.rol.nombre == "Aprendiz":
        ...
        if aprendiz:
            ...
            # CORREGIDO: getattr para no fallar si el modelo aún no tiene id_periodo
...
def solicitar_recuperacion(datos: SolicitarRecuperacionRequest, session: Session = Depends(get_session)):
    ...
    if usuario:
        ...
        print(f"[DEV] Enlace de recuperación para {usuario.correo}: http://localhost:5173/restablecer-contrasena?token={token}")
    return {"mensaje": "Si el correo está registrado, recibirás un enlace de recuperación en unos minutos."}
