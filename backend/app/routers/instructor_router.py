...
def crear_instructor(instructor: InstructorCreate, session: Session = Depends(get_session)):
    try:
        return InstructorService.crear(session, instructor)
...
def actualizar_instructor(instructor_id: int, instructor: InstructorUpdate, session: Session = Depends(get_session)):
    try:
